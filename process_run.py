    ...
    def ProcessEntry(self):
        ...
        while True:
            ...
            if len(b) < s1.size + s2.size + s3.size + array1.size + array2.size:
                break
            first_entry._make(s1.unpack_from(b, 0))
            first_array = [array1.unpack_from(b, 234)]
            numpy.reshape(first_array, (16, 512))
            second_entry._make(s2.unpack_from(b, 16618))
            second_array = [array2.unpack_from(b, 16746)]
            third_entry._make(s3.unpack_from(b, 20842))
            entry = TestEntry(first_entry, second_entry, third_entry, first_array, second_array)
            self.EntryList.append(entry)
        print "Run length: " + str(len(self.EntryList))
if __name__ == "__main__":
    run = TestRun("./data.bin")
    run.ProcessEntry()
